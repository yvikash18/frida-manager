...
static const std::vector<std::string> FRIDA_PATTERNS = {
    ...
    "zygisk", "lsposed", "/data/local/tmp", "/data/adb/"
};
...
Java_com_prapps_fridaserverinstaller_rasp_RaspDetector_nativeScanMaps(JNIEnv *env, jobject) {
    ...
    if (mapsFile.is_open()) {
        ...
        while (std::getline(mapsFile, line)) {
            ...
            // Check for anonymous executable memory (rwxp, r-xp, --xp)
            if ((line.find("rwxp") != std::string::npos || line.find("r-xp") != std::string::npos || line.find("--xp") != std::string::npos) && 
                (line.find("[anon:") != std::string::npos || line.length() < 75)) { // Heuristic for short lines being anon
                results.push_back("Anon Exec: " + line.substr(0, 60));
            }
        }
    }
